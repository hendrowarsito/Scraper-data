...
Strategi parsing:
  ...
  Pendekatan: httpx (request ringan) → BeautifulSoup cari __NEXT_DATA__ → parse JSON.
...
def fetch_page(url, timeout=15):
    """
    Mengambil HTML halaman dengan httpx.
    Mengembalikan (html_text, status_code) atau (None, error_msg).

    Catatan: httpx cukup untuk halaman listing rumah123 karena
    data listing di-inject via __NEXT_DATA__ di server-side (SSR),
    bukan di-load via JavaScript di browser (CSR).
    """
    try:
        with httpx.Client(headers=HEADERS, follow_redirects=True, timeout=timeout) as client:
            ...
            if resp.status_code == 200:
                ...
                return None, "403 — Akses ditolak (rate limit atau blokir IP)"
            elif resp.status_code == 404:
                return None, "404 — Halaman tidak ditemukan"
            else:
                return None, f"HTTP {resp.status_code}"
    except httpx.TimeoutException:
        return None, "Timeout — koneksi terlalu lambat"
